...
if ( -x "$prefix/$plugin" ) {
  ...
  $t += checkCmd( "$prefix/$plugin -F $prefix/xml/nagios-1.0.xml -H hostname -s service -i 1 -e P", 3, '/ERROR: Content Error: - Host: Host Name ... ne hostname - Service: Service Name ... ne service - Environment: LOCAL ne P/');
  $t += checkCmd( "$prefix/$plugin -F $prefix/xml/nagios-1.0.xml -H 'Host Name ...' -s service -i 10 -e P", 3, '/ERROR: Content Error: - Service: Service Name ... ne service - Environment: LOCAL ne P/');
  $t += checkCmd( "$prefix/$plugin -F $prefix/xml/nagios-1.0.xml -H 'Host Name ...' -s 'Service Name ...' -i 10 -e P", 3, '/ERROR: Content Error: - Environment: LOCAL ne P/');
  $t += checkCmd( "$prefix/$plugin -F $prefix/xml/nagios-1.0.xml -H 'Host Name ...' -s 'Service Name ...' -i 10 -e L", 3, '/Result into XML file \'[//\w\-\.]+\' are out of date:/');
} else {
