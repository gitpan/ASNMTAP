...
use ASNMTAP::Asnmtap::Plugins qw(:DEFAULT :ASNMTAP :PLUGINS :MAIL);

use Getopt::Long;
use vars qw($opt_t $opt_S $opt_D $opt_L $opt_d $opt_O $opt_A $opt_V $opt_h $PROGNAME);

# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

$PROGNAME    = "check_template-mail.pl";
my $prgtext  = "Mail plugin template for testing the '$APPLICATION'";
my $version  = "3.0";
$TIMEOUT     = 10;
...
my $mailStatus     = $textStatus . ' <' . $textStatusUp . '>';
...
if ( $returnCode ) {
  ...
  ($alert, $state, $result, $numberOfMails, $statusUp, $statusDown) = receiving_fingerprint_mails ( $alert, $state, $serverPOP3, $username, $password, $TIMEOUT, $emailsReceivedState, $textFrom, $textTo, $textSubject, $textStatus, $textStatusUp, $textStatusDown, $mailTo, $mailFrom, $mailSubject, $mailHeader, $mailPluginname, $mailBranding, $mailTimestamp, $mailStatus, 7, $debug, \&actionOnMailBody, $onDemand );
...
sub actionOnMailBody {
  my ($tNumberOfMails, $tAlert, $tState, $tResult, $tPop, $tMsgnum, $tDate, $tTime, $tDay, $tMonth, $tYear, $tHour, $tMin, $tSec, $tDebug, $tOnDemand) = @_;
...
sub printUsage () {
  print "Usage: $PROGNAME $PLUGINUSAGE\n";
}
