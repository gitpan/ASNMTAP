...
# ----------------------------------------------------------------------------------------------------------
# 2008/mm/dd, v3.000.017, create_weblogic_configuration_database_with_SNMP.pl
# ----------------------------------------------------------------------------------------------------------
...
if ( $dbh ) {
  ...
  while ( my ($domain, $hash) = each( %adminServers ) ) {
    while ( my ($key, $oid) = each( %serverTable ) ) { _snmpwalk ( \$objectPlugins, \%infoServers, $domain, $hash, $key, $oid, $callsystem, $debug ); }
    while ( my ($key, $oid) = each( %clusterTable ) ) { _snmpwalk ( \$objectPlugins, \%infoClusters, $domain, $hash, $key, $oid, $callsystem, $debug ); }
  }
...
sub _snmpwalk {
  my ($asnmtapInherited, $infoHASH, $domain, $hash, $key, $oid, $call_system, $debug) = @_;
