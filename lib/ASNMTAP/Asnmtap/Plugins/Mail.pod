...
Syntax for POP3 host is <hostname>[:<port>] where <hostname> can be a hostname or the IP-address on the 'xxx.xxx.xxx.xxx'
 ...
 scalar array : pop3 => [ qw(pop3.citap.be pop3.citap.com) ]  <-<-<- TODO
...
optional, scalar value, default: 'ASNMTAP'
...
optional, when you need to pass parameters to your own custom
defined function, this can be done with customArguments.
...
the first number of lines of the body for the message.

optional, scalar value, default 7

=item receivedState

when receivedState == 1:
  No emails received => $ERRORS{OK} otherwise $ERRORS{WARNING..UNKNOWN}

when receivedState == 0:
  x mail(s) received => $ERRORS{OK} otherwise $ERRORS{WARNING..UNKNOWN}
