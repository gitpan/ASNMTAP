...
=head1 SYNOPSIS
  ...
  $objectWebTransact->check ( {}, asnmtapInherited => \$objectPlugins, newAgent => 1 );
...
hash, { var_label_1 => EXP_VAR_LABEL_1, ... }
...
When we specify at the command prompt I<--proxy=username;password@proxyserver:port>, the needed and required proxy settings are automatically set.
...
The first time we call $objectWebTransact->check ( {}, ... ), then newAgent must equals 1, so that an (browser) session will be created.
...
Is an scalar, value is number, default: 0
...
Exp_Fault field the check fails.
  ...
  my $objectWebTransact = ASNMTAP::Asnmtap::Plugins::WebTransact->new ($ar);
  my $returnCode = $objectWebTransact->check ( { tmno => $tmno }, asnmtapInherited => \$objectPlugins, newAgent => 1 );
...
the query string p_tmno1 = <current_value_of_arg_hash{'tmno'}>
  ...
  $objectWebTransact = ASNMTAP::Asnmtap::Plugins::WebTransact->new ( URLS );
  $returnCode = $objectWebTransact->check ( {tmno => $tmno}, asnmtapInherited => \$objectPlugins, newAgent => 1 )
...
=item Example 4
  ...
  $returnCode = $objectWebTransact->check ( {}, custom => \&customWebTransact, asnmtapInherited => \$objectPlugins );
...
=item * B<you> must identify the URLs and the query strings required by the transaction 
