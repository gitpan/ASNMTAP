...
# ----------------------------------------------------------------------------------------------------------
# 2006/02/28, v3.000.004, package ASNMTAP::Asnmtap::Applications::CGI Object-Oriented Perl
# ----------------------------------------------------------------------------------------------------------
...
use ASNMTAP::Asnmtap::Applications qw(:DEFAULT :ASNMTAP :CGI);
...
our @EXPORT_OK   = qw($CAPTUREOUTPUT $PREFIXPATH $PLUGINPATH $PERLCOMMAND $RSYNCCOMMAND $SCPCOMMAND $SSHCOMMAND %ERRORS %STATE &get_datetimeSignal &scan_socket_info &sending_mail
                      $APPLICATIONPATH
...
our %EXPORT_TAGS = (ASNMTAP     => [qw($CAPTUREOUTPUT $PREFIXPATH $PLUGINPATH $PERLCOMMAND $RSYNCCOMMAND $SCPCOMMAND $SSHCOMMAND %ERRORS %STATE &get_datetimeSignal &scan_socket_info &sending_mail)],
                    ...
                    SADMIN      => [qw(%COLORSTABLE %ICONSRECORD
                                       $SSHLOGONNAME $RSYNCIDENTITY $SSHIDENTITY $WWWIDENTITY
                                       $RMVERSION $RMDEFAULTUSER
                                       &get_csvfiledate &get_csvfiletime)],
        ...
        if (defined $errorUserAccessControl) {
          ...
          if ( $ENV{'REMOTE_ADDR'} ) {
            if ( $CremoteAddr ne "" ) {
              ...
              $errorUserAccessControl = "IP Address Forbidden." if (! $ipRange->contains($ipAddr));
            }
          }
		  ...
		  if (! defined $errorUserAccessControl) {
            ...
            $errorUserAccessControl = "You are onto the wrong place to be!" if ( ! $accessGranted );
...
sub get_sql_startDate_sqlEndDate_numberOfDays_test {
  ...
  return (0, undef, undef, undef) if (! (check_date($fromYear, $fromMonth, $fromDay) and check_date($toYear, $toMonth, $toDay)));
