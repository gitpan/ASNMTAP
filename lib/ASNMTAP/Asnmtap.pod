...
=head1 OVERVIEW OF CONSTANTS
 ...
 $PREFIXPATH        /opt/asnmtap-3.000.xxx
...
The default location for the config file is into the root for ASNMTAP-based applications and plugins '/opt/asnmtap-3.000.xxx'.
...
returned the get option type (':' or '=') or undef when item don't exists
...
ALL
  ...
  $APPLICATION $BUSINESS $DEPARTMENT $COPYRIGHT $SENDEMAILTO $TYPEMONITORING
  $CAPTUREOUTPUT
