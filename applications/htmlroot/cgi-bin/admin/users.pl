...
# ---------------------------------------------------------------------------------------------------------
# 2006/03/18, v3.000.006, users.pl for ASNMTAP::Asnmtap::Applications::CGI making Asnmtap v3.000.xxx compatible
# ---------------------------------------------------------------------------------------------------------
...
if ( defined $sessionID and ! defined $errorUserAccessControl ) {
  ...
  if ($dbh and $rv) {
    ...
    if ($action eq "duplicateView" or $action eq "insertView") {
      ...
      ($rv, $matchingUsers) = check_record_exist ($rv, $dbh, $sql, 'Comments', 'Remote User', 'Title', '', $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);
      ...
      $sql = "select count(*) from $SERVERTABLUSERS";
      ($rv, $numberRecordsIntoQuery) = do_action_DBI ($rv, $dbh, $sql, $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);
      ...
      $sql = "select remoteUser, givenName, familyName, userType, activated from $SERVERTABLUSERS order by $orderBy limit $pageOffset, $RECORDSONPAGE";
      ...
      ($rv, $matchingUsers, $nextAction) = record_navigation_table ($rv, $dbh, $sql, 'User', 'remoteUser', '0', '', '3#0=>Guest|1=>Member|2=>Moderator|4=>Administrator|8=>Server Administrator', '', $orderBy, $header, $navigationBar, $iconAdd, $iconDelete, $iconDetails, $iconEdit, $nextAction, $pagedir, $pageset, $pageNo, $pageOffset, $htmlTitle, $subTiltle, $sessionID, $debug);
    }
    ...
    if ($action eq "deleteView" or $action eq "displayView" or $action eq "duplicateView" or $action eq "editView" or $action eq "insertView") {
      ...
      ($rv, $keyLanguageSelect, undef) = create_combobox_from_DBI ($rv, $dbh, $sql, 1, '', $CkeyLanguage, 'keyLanguage', 'none', '-Select-', $formDisabledAll, '', $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);
...
function validateForm() {
  ...
  var objectRegularExpressionRemoteUserValue = /\^[a-z|A-Z|0-9|-]\+\$/;
...
HTML
    ...
    if ( $iconAdd ) {
      print <<HTML;
        <td class="StatusItem"><a href="$urlWithAccessParameters&amp;action=insertView&amp;orderBy=$orderBy">[Insert new user]</a></td>
