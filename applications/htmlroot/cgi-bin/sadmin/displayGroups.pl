...
# ---------------------------------------------------------------------------------------------------------
# 2006/04/xx, v3.000.007, displayGroups.pl for ASNMTAP::Asnmtap::Applications::CGI making Asnmtap v3.000.xxx compatible
# ---------------------------------------------------------------------------------------------------------
# COPYRIGHT NOTICE
...
my ($sessionID, $iconAdd, $iconDelete, $iconDetails, $iconEdit, $iconQuery, $iconTable, $errorUserAccessControl, undef, undef, undef, undef, undef, undef, undef, undef, undef, undef, undef, $subTiltle) = user_session_and_access_control (1, 'admin', $cgi, $pagedir, $pageset, $debug, $htmlTitle, "Display Groups", undef);
...
if ( defined $sessionID and ! defined $errorUserAccessControl ) {
  ...
  if ($dbh and $rv) {
    ...
    if ($action eq "duplicateView" or $action eq "insertView") {
      ...
      $sql = "select displayGroupID from $SERVERTABLDSPLYGRPS WHERE displayGroupID='$CdisplayGroupID'";
	  ...
	  if ( $numberRecordsIntoQuery ) {
        ...
        $sql = 'INSERT INTO ' .$SERVERTABLDSPLYGRPS. ' SET groupTitle="' .$CgroupTitle. '", activated="' .$dummyActivated. '"';
      ...
      ($rv, $matchingDisplayGroups) = check_record_exist ($rv, $dbh, $sql, 'Views', 'Unique Key', 'Display Daemon', '', $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);

	  if ($matchingDisplayGroups eq "") {
        $htmlTitle = "Display Group $CdisplayGroupID deleted";
        $sql = 'DELETE FROM ' .$SERVERTABLDSPLYGRPS. ' WHERE displayGroupID="' .$CdisplayGroupID. '"';
        $dbh->do ( $sql ) or $rv = error_trap_DBI(*STDOUT, "Cannot dbh->do: $sql", $debug, $pagedir, $pageset, $htmlTitle, $subTiltle, 3600, "", $sessionID);
      } else {
      ...
      my $dummyActivated = ($Cactivated eq "on") ? 1 : 0;
      $sql = 'UPDATE ' .$SERVERTABLDSPLYGRPS. ' SET displayGroupID="' .$CdisplayGroupID. '", groupTitle="' .$CgroupTitle. '", activated="' .$dummyActivated. '" WHERE displayGroupID="' .$CdisplayGroupID. '"';
      $dbh->do ( $sql ) or $rv = error_trap_DBI(*STDOUT, "Cannot dbh->do: $sql", $debug, $pagedir, $pageset, $htmlTitle, $subTiltle, 3600, "", $sessionID);
      $nextAction   = "listView" if ($rv);
      ...
      $sql = "select count(*) from $SERVERTABLDSPLYGRPS";
      ($rv, $numberRecordsIntoQuery) = do_action_DBI ($rv, $dbh, $sql, $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);
      ...
      $sql = "select displayGroupID, groupTitle, activated from $SERVERTABLDSPLYGRPS order by $orderBy limit $pageOffset, $RECORDSONPAGE";
      ...
      ($rv, $matchingDisplayGroups, $nextAction) = record_navigation_table ($rv, $dbh, $sql, 'Display Group', 'displayGroupID', '0', '0', '', '', $orderBy, $header, $navigationBar, $iconAdd, $iconDelete, $iconDetails, $iconEdit, $nextAction, $pagedir, $pageset, $pageNo, $pageOffset, $htmlTitle, $subTiltle, $sessionID, $debug);
    }

    if ($action eq "deleteView" or $action eq "displayView" or $action eq "duplicateView" or $action eq "editView") {
      $sql = "select displayGroupID, groupTitle, activated from $SERVERTABLDSPLYGRPS where displayGroupID='$CdisplayGroupID'";
      ...
      if ( $rv ) {
        ($CdisplayGroupID, $CgroupTitle, $Cactivated) = $sth->fetchrow_array() or $rv = error_trap_DBI(*STDOUT, "Cannot $sth->fetchrow_array: $sql", $debug, $pagedir, $pageset, $htmlTitle, $subTiltle, 3600, "", $sessionID) if ($sth->rows);
