...
# ---------------------------------------------------------------------------------------------------------
# 2007/06/10, v3.000.014, pagedirs.pl for ASNMTAP::Asnmtap::Applications::CGI
# ---------------------------------------------------------------------------------------------------------
...
my ($sessionID, $iconAdd, $iconDelete, $iconDetails, $iconEdit, $iconQuery, $iconTable, $errorUserAccessControl, undef, undef, undef, undef, undef, undef, undef, undef, undef, undef, undef, $subTiltle) = user_session_and_access_control (1, 'admin', $cgi, $pagedir, $pageset, $debug, $htmlTitle, "Pagedirs", undef);
...
if ( defined $sessionID and ! defined $errorUserAccessControl ) {
  ...
  if ($dbh and $rv) {
    ...
    if ($action eq "duplicateView" or $action eq "insertView") {
      ...
      $sql = "select uKey, concat( LTRIM(SUBSTRING_INDEX(title, ']', -1)), ' (', $SERVERTABLENVIRONMNT.label, ')' ) from $SERVERTABLPLUGINS, $SERVERTABLENVIRONMNT where pagedir REGEXP '/$Cpagedir/' and $SERVERTABLPLUGINS.environment = $SERVERTABLENVIRONMNT.environment order by title, uKey";
      ($rv, $matchingPagedirs) = check_record_exist ($rv, $dbh, $sql, 'Plugins', 'Unique Key', 'Title', '', $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);

      $sql = "select remoteUser, email from $SERVERTABLUSERS where pagedir REGEXP '/$Cpagedir/' order by email";
      ($rv, $matchingPagedirs) = check_record_exist ($rv, $dbh, $sql, 'Users', 'Remote User', 'Email', $matchingPagedirs, $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);
	
      $sql = "select pagedir, groupName from $SERVERTABLDSPLYDMNS where pagedir = '$Cpagedir' order by groupName";
      ($rv, $matchingPagedirs) = check_record_exist ($rv, $dbh, $sql, 'Display Daemons', 'Pagedir', 'Group Name', $matchingPagedirs, $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);
      ...
      $sql = "select count(pagedir) from $SERVERTABLPAGEDIRS";
      ($rv, $numberRecordsIntoQuery) = do_action_DBI ($rv, $dbh, $sql, $pagedir, $pageset, $htmlTitle, $subTiltle, $sessionID, $debug);
      ...
      ($rv, $matchingPagedirs, $nextAction) = record_navigation_table ($rv, $dbh, $sql, 'Pagedir', 'pagedirs', '0', '', '', '', $orderBy, $header, $navigationBar, $iconAdd, $iconDelete, $iconDetails, $iconEdit, $nextAction, $pagedir, $pageset, $pageNo, $pageOffset, $htmlTitle, $subTiltle, $sessionID, $debug);
    }
    ...
    if ($action eq "deleteView" or $action eq "displayView" or $action eq "duplicateView" or $action eq "editView") {
      ...
      if ( $rv ) {
        ($Cpagedir, $CgroupName, $Cactivated) = $sth->fetchrow_array() or $rv = error_trap_DBI(*STDOUT, "Cannot $sth->fetchrow_array: $sql", $debug, $pagedir, $pageset, $htmlTitle, $subTiltle, 3600, '', $sessionID) if ($sth->rows);
