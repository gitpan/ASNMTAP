...
# ---------------------------------------------------------------------------------------------------------
# 2006/04/xx, v3.000.007, generatePluginCrontabSchedulingReport.pl for ASNMTAP::Asnmtap::Applications::CGI making Asnmtap v3.000.xxx compatible
# ---------------------------------------------------------------------------------------------------------
...
if ( $rv ) {
  ...
  if ( $rv ) {
    ...
    $chartTitle = "$prgtext for '$applicationTitle'";
    my ($mon, $mday) = (1, 1);
    @labels = ();
